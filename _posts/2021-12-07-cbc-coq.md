...
4. **Consensus values**: A multi-element set, $\mathcal{C}$, e.g., in binary consensus protocol, $\mathcal{C}=\left{0,1\right}$, while in a blockchain consensus protocol, $\mathcal{C}$ is the set of all possible blockchains.
   ```coq
   consensus_values : Type;
