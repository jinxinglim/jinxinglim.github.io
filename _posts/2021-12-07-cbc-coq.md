...
5. **Estimator**, `E` (Definition 2.5 from [1]) A function $\varepsilon: \Sigma \rightarrow \mathcal(P)(\mathcal{C}) \setminus \emptyset$, which relates CBC Casper protocol states, $\Sigma$, (we will define this later) to the sets of consensus values, in particular, powerset of $\mathcal{C}, \mathcal{P}(\mathcal{C})$ excluding the empty set, $\emptyset$
   ...
   ```
   > **Remark**: The Coq code presented above uses two different terms, `state` and `prot_state`. `state` represents protocol state as seen in [1] and `prot_state` represents protocol state with equivocation fault tolerance `t` (more on this later).
...
2. The functions, such as `weight`, `t` and `E`, are defined as proposition type instead of the regular function type, where we can use `Definition` and/or `Fixpoint` to define in Coq. The purpose of doing so is better facilitate the proving processes of the formal proofs which we will see later on. Defining them as functions does not guarantee the correctness of their specifications. On the other hand, when defining them as propositions guarantees their correctness (thats why properties mentioned in point 1 above must be defined) and can be used easily in the long run when proving the safety properties of the CBC Casper protocols.
...
1. **States**, `state`:
   ...
   ```
   > **Remark**: The `state` in `CBCProtocolEq` refers to "protocol state" in [1].
   
2. **Reachibility Relation**, `reach` (Combines Definition 2.7 and 2.8 of [1]):
   ```coq
   reach : state -> state -> Prop;
   (** reach defines reachibility relation that takes in 2 (input) states and output a reachibility proposition **)
   reach_refl : forall s, reach s s;
   (** reach is reflexive **)
   reach_trans : forall s1 s2 s3, reach s1 s2 -> reach s2 s3 -> reach s1 s3;
   (** reach is transitive **)
...
4. **Equivocation**, `equivocating_weight` (Definition 2.10 of [1]):    
...
4. **Protocol states**, `prot_state` (Combines Definition 2.8 and 2.12 of [1])
   ```coq
   prot_state : state -> Prop;
   (** prot_state takes an input state and output a proposition that the input state is a prot_state **)
   about_state0 : prot_state state0;
   (** about_state0 tells us that the initial state0 is a prot_state **)
   about_prot_state : forall s1 s2, prot_state s1 -> prot_state s2 -> 
                        (equivocation_weight (state_union s1 s2) <= proj1_sig t)%R -> prot_state (state_union s1 s2);
   (** about_prot_state tells us that for any states s1 s2, if s1 and s2 are prot_state, and equivocation_weight of (state_union s1 s2) is less than or equal to (some predefined) t, then (state_union s1 s2) is a prot_state **)
   ```
   > **Remark**: The `prot_state` in `CBCProtocolEq` refers to Definition 2.12 of [1], which is protocol state with equivocation fault tolerence `t`.

There are also several definitions and properties that need to formally define before moving on to the safety proof presented in [1]. Some of these properties seem trivial for humans, but they are required to formally defined in Coq to allow the proofs to be formally verified. As we are not going in depth to the theorems (5 in total) of the safety proof, we will just be briefly explaining these properties (through comments in code snippets).

...TO BE COMPLETED...

With all the necessary formal definitions of the data and properties defined for the typeclass `CBCProtocolEq`, we are ready to move on to the safety proofs as described in Section 2.3 of [1].
...
[5]. [Safety Proofs for the minimal CBC Casper family](https://zunction.github.io/blog/2021/safety-proofs/)

[6]. [Formalizing Correct-by-Construction in Coq](https://zunction.github.io/blog/2020/formalizing-correct-by-construction-casper-coq/)
