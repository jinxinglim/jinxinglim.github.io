...
4. **Consensus values** (Definition 2.4 from [1]): A multi-element set, $\mathcal{C}$, e.g., in binary consensus protocol, $\mathcal{C}=\\{0,1\\}$, while in a blockchain consensus protocol, $\mathcal{C}$ is the set of all possible blockchains.
   ```coq
   consensus_values : Type;
